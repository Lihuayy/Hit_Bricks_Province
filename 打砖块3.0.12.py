...
class Game:
    ...
    def create_bricks(self):
        self.bricks.empty()
        for y in range(5):
            for x in range(WIDTH // (BRICK_WIDTH + 5)):
                # 为每块砖块生成一个随机颜色
                brick_color = (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
                brick = Brick(x * (BRICK_WIDTH + 5) + 5, y * (BRICK_HEIGHT + 5) + 50, brick_color)
    ...
    def login(self):
        ...
        while not self.logged_in:
            ...
            self.draw_text("请输入用户名：", WIDTH // 2, HEIGHT // 2 - 80)
            self.draw_text(text, WIDTH // 2, HEIGHT // 2 - 20)
            pygame.display.flip()
            self.clock.tick(60)
    ...
    def play(self):
        ...
        while running:
            ...
            if not self.paused and not self.game_over:  # 确保只在游戏进行时更新
                self.update()
